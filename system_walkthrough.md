...
```python
    ...
    ...
```
**Explanation:** Provides a drag-and-drop file uploader. When a file is uploaded, it checks the extension and calls the corresponding file extraction function from `document_processor.py`.
...
## 3. Database & Retrieval: `retrieval.py`
...
```
**Explanation:** The second node. Takes the summarized `current_query` generated by the previous node, passes it to `retrieval.py` functions to search ChromaDB, and returns the list of matched `candidates`.
